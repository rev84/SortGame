...
Env = (function() {
  ...
  Env.character2answer = function(obj) {
    var myIndex, offset, targetIndex;
    myIndex = $(obj).data('index');
    if (0 <= Env.answerIndexes.indexOf(myIndex)) {
      return;
    }
    targetIndex = Env.answerIndexes.length;
    offset = Env.answerSpan[targetIndex].offset();
    Env.bodySpan[myIndex].animate(offset);
    return Env.answerIndexes.push(myIndex);
  };

  Env.answer2character = function() {
    var myIndex, offset, targetIndex;
    myIndex = Env.answerIndexes.length - 1;
    if (myIndex < 0) {
      return;
    }
    targetIndex = Env.answerIndexes[myIndex];
    offset = Env.characterSpan[targetIndex].offset();
    Env.bodySpan[targetIndex].animate(offset);
    return Env.answerIndexes.pop();
  };
  ...
  Env.onResize = function() {
    var index, j, l, len, len1, ref, ref1, results;
    ref = Env.answerIndexes;
    for (j = 0, len = ref.length; j < len; j++) {
      index = ref[j];
      if (index !== null) {
        Env.bodySpan[index].offset(Env.answerSpan[index].offset());
      }
    }
    ref1 = Env.characterIndexes;
    results = [];
    for (l = 0, len1 = ref1.length; l < len1; l++) {
      index = ref1[l];
      if (index !== null) {
        results.push(Env.bodySpan[index].offset(Env.characterSpan[index].offset()));
      } else {
        results.push(void 0);
      }
    }
...
initQuestion = function(description, answer) {
  ...
  for (index = l = 0, ref = answerArray.length; 0 <= ref ? l < ref : l > ref; index = 0 <= ref ? ++l : --l) {
    ...
    bodySpan.on('click', function() {
      return Env.character2answer(this);
    });
    $('body').append(bodySpan);
    bodySpan.offset(Env.characterSpan[index].offset());
    results.push(Env.bodySpan.push(bodySpan));
  }
  return results;
};
